...
def main(run:Run):
    ...
    run_pattern = "./post_training_sft_results/*"
    run_dirs = glob.glob(run_pattern)
    
    if not run_dirs:
        print("No run directories found in ./post_training_sft_results/")
        return
            ...
            for benchmark_name, benchmark_results in results["results"].items():
                for metric_name, metric_value in benchmark_results.items():
                    if metric_name != "alias":  # Skip the alias field
                        ...
                        flattened_results[f"eval/benchmark/{benchmark_name}/{clean_metric_name}"] = metric_value
...
if __name__ == "__main__":
    ...
    run = Run(
        ...
        run_id=run_id # If run exists, then data will be logged to the existing run
    )   
    main(run)
