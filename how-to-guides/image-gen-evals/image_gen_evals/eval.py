...
from image_gen_evals.lib.checkpoints import (
    load_checkpoint_recursive,
    get_run_lineage_metadata,
)
...
def evaluate(
    ...
    # Get run metadata recursively
    print(f"Fetching metadata for run {run_id}...")
    experiment_name, fork_run_id, fork_step = get_run_lineage_metadata(project, run_id)
    
    if not experiment_name:
        print(f"❌ Could not retrieve experiment name for run {run_id}")
        return

    # Load checkpoint recursively through lineage
    try:
        print(f"Loading checkpoint recursively from {run_id=} {step=}")
        pipeline, training_info = load_checkpoint_recursive(project, run_id, step, device=device)
        pipeline.unet.eval()
        print(
