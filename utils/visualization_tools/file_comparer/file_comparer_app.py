...
def download_neptune_files(
    ...
    try:
        # List experiments
        exps = nq.list_experiments(project=project_name, experiments=f"{experiment_regex}")
...
def main():
    ...
    with st.sidebar.expander("📂 File Configuration", expanded=expander_open):
        if not NEPTUNE_AVAILABLE:
            ...
            # Neptune project
            _neptune_project = st.session_state.get("neptune_project") or os.getenv(
                "NEPTUNE_PROJECT"
            )
            neptune_project = st.text_input(
                ...
                help="In the format `workspace_name/project_name`",
            )
            ...
            download_directory = st.text_input(
                ...
                value=st.session_state.get(
                    "download_directory", os.path.join(os.getcwd(), "neptune_downloads")
                ),
            ...
            if st.button("⬇️ Fetch and Visualize"):
                with st.spinner("Downloading files from Neptune..."):
                    ...
                    if files:
                        ...
                        st.warning(
                            "No files were downloaded. Check your project name and regex patterns."
                        )
    ...
    if "files" in st.session_state and st.session_state.files:
        ...
        layout_orientation = st.sidebar.toggle(
            "Steps as columns", value=True, help="Choose how to arrange steps in the gallery"
        )
    ...
    if "files" not in st.session_state or not st.session_state.files:
        st.info(
            "👆 Please configure your Neptune project and click 'Fetch and Visualize' to get started"
        )
        return
            ...
            with col4:
                ...
                if st.button("➡️ Next", disabled=not can_move_next):
                    ...
                    st.session_state.current_step_index = min(
                        total_steps - images_per_page, current_step_index + 1
                    )
                    st.rerun()
